...
class AbayBankScraper(BaseScraper):
    """
    Scraper for Abay Bank foreclosure auctions.
    """
    
    base_url = "https://www.abaybanksc.com"
    source_name = "Abay Bank"
    ...
    def scrape(self) -> ScrapeResult:
        ...
        try:
            self.logger.info("Starting Abay Bank scrape")
            
            auction_paths = ["/tenders", "/category/auction", "/የሐራጅ-ሽያጭ"]
            ...
            self.logger.error(f"Error during Abay Bank scrape: {e}")
    ...
    def _parse_card_element(self, card) -> ScrapedListing:
        ...
        try:
            ...
            title = title_elem.get_text(strip=True) if title_elem else "Abay Bank Auction"
