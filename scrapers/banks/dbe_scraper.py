"""
Development Bank of Ethiopia (DBE) auction scraper.
"""
import logging
import re
...
class DBEScraper(BaseScraper):
    """
    Scraper for Development Bank of Ethiopia foreclosure auctions.
    """
    
    base_url = "https://dbe.com.et"
    ...
    def scrape(self) -> ScrapeResult:
        ...
        try:
            self.logger.info("Starting DBE scrape")
            
            auction_paths = ["/tenders", "/notice", "/auction"]
            ...
            self.logger.error(f"Error during DBE scrape: {e}")
    ...
    def _extract_listings_from_page(self, soup, result):
        ...
        cards = soup.select('article, .post, .entry, tr, .tender-row, .tender-item')
        
        for card in cards:
            text = card.get_text()
            if any(kw in text for kw in ["ሐራጅ", "Auction", "Tender", "ጨረታ"]):
    ...
    def _parse_card_element(self, card) -> ScrapedListing:
        ...
        try:
            ...
            title = title_elem.get_text(strip=True) if title_elem else "DBE Auction"
